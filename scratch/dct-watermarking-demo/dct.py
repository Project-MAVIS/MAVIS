...
class WaveletDCTWatermark:
    def __init__(self, image: Optional[UploadedFile] = None):
    ...
    def fwatermark_image(self, image, watermark):
        """Main watermarking process using image as a file"""
        try:
            model = 'haar'
            level = 1
            ...
            watermark_array = self.convert_image(watermark, 128, to_grayscale=True)
    ...
    def frecover_watermark(self, image, model='haar', level=1):
        """Recover watermark from watermarked image with enhanced clarity"""
        try:
